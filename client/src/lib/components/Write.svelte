...
<form onsubmit={async e=>{
  ...
  <textarea name="content" id="content" placeholder="Content" required cols="30" rows="10"></textarea>
  <div class="flex flex-row flex-wrap gap-2">
    <div class="flex flex-col gap-1">
      <label for="cat">Category</label>
      <input type="text" name="cat" id="cat" placeholder="Category" class="input-sm">
    </div>
    <div class="flex flex-col gap-1">
      <label for="created">Created<span class="text-red-300">*</span></label>
      <input type="datetime-local" required name="created" id="created" value={new Date().toISOString().slice(0, -8)} class="input-sm">
    </div>
  </div>
  <button class="btn text-lg">Create</button>
</form>
